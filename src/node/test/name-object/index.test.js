...
test('validate name-counter identity invariant', async () => {
  await expect(parseNameObjects({
    // * When this function is given `{ nameStem: 'Z', nameCounterValue: 2 }`,
    //   then it will return `Z-AA`.
    // * When this function is given `{ nameStem: 'Z', nameCounterValue: 1 }`,
    //   then it will return `Z-A`.
    deriveName ({ nameStem, nameCounterValue }) {
      return `${nameStem}-${'A'.repeat(nameCounterValue)}`;
    },

    // * When this function is given `'ZA-A'`, then it will return `{ nameStem:
    //   'ZA', nameCounterValue: 1 }`.
    // * When this function is given `'ZAA'`, then it will return `{ nameStem:
    //   'Z', nameCounterValue: 2 }`.
    parseName (name) {
      switch (name) {
        case 'ZA-A':
          return { nameStem: 'ZA', nameCounterValue: 1 };
        case 'ZAA':
          return { nameStem: 'Z', nameCounterValue: 2 };
      }
      return {
        nameStem: 'Z', nameCounterType: 'TEST', nameCounterValue: 2,
      };
    },

    // The following line will be parsed into the name object `{
    // headScalarRangeInitial: 0, nameStem: 'ZA', nameCounterValue: 1 }`. This
    // is because of the preceding given definition of `parseName`.
    unicodeDataLines: [ '0000;ZA-A' ],

    // However, that name object will violate the name-counter identity
    // invariant between the given `deriveName` and the given `parseName`.
    //
    // This is because that name object’s derived name (as defined by the given
    // `deriveName` function) is `'ZA-A'`, which is `'ZAA'` when fuzzily folded.
    // However, `'ZAA'` itself would parse into the name object `{ nameStem:
    // 'Z', nameCounterValue: 2 }`.
    //
    // That name object and the original name object do not match! (Their name
    // stems are `'ZA'` versus `'Z'`, and their name-counter values are `1`
    // versus `2`).
    //
    // If we used this name object and these definitions of `deriveName` and
    // `parseName` to create a database, then attempting to look up the
    // character by its name `ZA-A` (or its fuzzily folded name `ZAA`) will
    // always fail. The database will use the fuzzily folded name (or, rather,
    // the fuzzily folded name’s name object) to search for matches, none of
    // which the database will be able to find.

    nameAliasesLines: [],
    namedSequencesLines: [],
  })).rejects.toThrow(/name-counter identity invariant/);
});
